...
class Stick {
    ...
    static WAIT         = 100;
    static READ_STEP    = 0.0037;
    static READ_OFFSET  = -153;
    static SAMPLES      = 20;
    static name         = "Stick";
    ...
    constructor(_pulse_pin, _read_pin) {
        ...
        read_pin = _read_pin;
        pulse_pin.configure(PWM_OUT, 1.0/1000000.0, 0.0); //changing frequency changes range of readings
...
function connection_handler(reason) {
    ...
    if(reason == SERVER_CONNECTED) {
        ...
        ::nv["reason"].append(reason); 
        ::nv["r_time"].append(time());
...
function update() {
    if (server.isconnected()) {
        ...
        server.connect(connection_handler, 10);
    }
}
