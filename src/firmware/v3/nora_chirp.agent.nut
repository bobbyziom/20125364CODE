...
#require "MongoLab.class.nut:2.0.0"
...
// mongo credentials
const API_KEY               = "TiKDiG-ItSvRYqlDjPtl9PVkYPhbRFG_";
const ACTIVE_DATABASE_NAME  = "managementsys";
...
db          <- MongoLab(API_KEY, ACTIVE_DATABASE_NAME);
...
device.on("report", function(data) {
    
    local _query;
    local _data = data;
    //local _time = _data.time;
    
    // prepare timestamp with ms for angular 
    //_time.apply(function(value){return (value.tointeger()*1000)});
    
    //server.log(http.jsonencode(_time));
    
    //_data.time <- _time;

    db.find("devices", { "id": SETUP.id }, function(err, resp, device) {
        ...
        db.update("devices", false, _query, { 
                 "$push": { "error" : { "$each": _data.error }, "time": { "$each": _data.time } }
